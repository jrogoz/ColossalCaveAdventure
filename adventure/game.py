...
class Game(Data):
    ...
    def score_and_exit(self):
        ...
        if i < len(self.class_messages) - 1:
            ...
            text_output = 'To achieve the next higher rating, you need {} more point{}\n'.format(d, 's' if d > 1 else '')
            self.write(text_output)
            synthesis(text_output)
        else:
            text_output = 'To achieve the next higher rating would be a neat trick!\n\nCongratulations!!\n'
            self.write(text_output)
            synthesis(text_output)
        self.is_done = True
